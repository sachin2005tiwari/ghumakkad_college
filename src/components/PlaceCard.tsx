...
const PlaceCard: React.FC<PlaceCardProps> = ({ place, onMouseEnter, onMouseLeave }) => {
 return (
 <Link 
...
// ⬆️ BOX-SHADOW CUSTOMIZATION ⬆️
 onMouseEnter={() => onMouseEnter(place.landingImage)}
onMouseLeave={onMouseLeave}
>
<img 
 src={place.landingImage} 
 alt={place.name} 
 className="w-full h-52 object-cover" 
 />
 <div className="p-4">
 <h2 className="text-xl font-medium mb-2 text-gray-800">{place.name}</h2>
 <p className="text-gray-600 text-sm">{place.landingDescription}</p>
</div>
</Link>
 );
};
