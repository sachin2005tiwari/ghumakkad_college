...
const PlaceDetailsPage: React.FC = () => {
	...
	// Fetches paginated comments and their likes
	useEffect(() => {
		if (!id) return;
		...
		const fetchCommentsAndLikes = async () => {
			...
			try {
				...
				// Your service returns { data, count }
				...
				// 2. Fetch like counts
				// Your service returns data directly
				...
				// Don't set state, let it remain empty
			} finally {
				setLoadingComments(false);
			}
		};
	...
	const handleToggleHeart = async (commentId: number) => {
		...
		// Call service (no need for try/catch, service handles it)
		try {
	...
	const handleAddComment = async () => {
		...
		try {
			...
			// We use the 'username' from Redux store since your service doesn't join
